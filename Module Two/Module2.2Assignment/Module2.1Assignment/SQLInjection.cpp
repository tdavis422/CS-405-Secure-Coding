...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //Create local version of SQL query to transform into lowercase version for regex to compare
    std::string localVersion(sql);
    std::transform(localVersion.begin(), localVersion.end(), localVersion.begin(), ::tolower);

    //Check for "OR value=value" SQL injection
    std::regex pattern("or\\s([0-9]+)=\\1|(['\"][\\w]+['\"]+)=\\2");
    std::smatch match;

    //Check if sql query matches regex pattern
    //Display warning and end methof if match confirm
    if (std::regex_search(localVersion, match, pattern) && match.size() > 1) {
        std::cout << "SQL Injection Suspected. Ending Method." << std::endl;
        return false;
    }
