...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (increment >= 0 && result > max - increment) {
            throw std::overflow_error("Numeric overflow has happened!");
        }
        else if (increment < 0 && result < min - increment) {
            throw std::underflow_error("Numeric underflow has happened!");
        }
...
void test_overflow()
{
    ...
    try {
        ...
        std::cout << +result; //displays result of adding method call
    }
    catch (const std::overflow_error& ex) { //catch if overflow exception occurs
        std::cout << ex.what(); //display error to user
    }
    catch (const std::underflow_error& ex) { //catch if underflow exception occurs
        std::cout << ex.what(); //display error to user
    }
    catch (const std::exception& ex) { //catch if different exception occurs
        std::cout << "Unknown issue" << ex.what(); //display error to user
    }
...
void test_underflow()
{
    ...
    try {
        ...
        std::cout << +result; //displays result of subracting method call
    }
    catch (const std::overflow_error& ex) { //catch if overflow exception occurs
        std::cout << ex.what(); //display error to user
    }
